...
### Зонная передача (AXFR)

Запрос `dig axfr` завершился с ошибкой `Transfer failed.`

Зонная передача — это механизм репликации DNS-зон между серверами. Успешная анонимная зонная передача позволила бы злоумышленнику получить полный список всех DNS-записей домена, что дало бы полную карту инфраструктуры сети. Тот факт, что сервер отклонил запрос, говорит о том, что он настроен безопасно и не раскрывает информацию посторонним.
...
PTR-запись нужна для обратного преобразования IP-адреса в имя хоста. Её отсутствие не является критичным для работы домена, но нарушает обратную связность и может вызывать задержки или ошибки в некоторых сервисах, которые полагаются на обратный DNS.
