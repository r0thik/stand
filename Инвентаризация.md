...
```mermaid
flowchart LR
    Kali[Kali Linux<br>192.168.203.128] <--> KaliNet[Сеть Kali<br>192.168.203.0/24]
    KaliNet <--> Router[Ubuntu Router<br>шлюз 192.168.203.1]
    Router --> Unknown[?]
```
...
flowchart LR
    ...
    subgraph Unknown [ ]
        direction LR
        Router[Ubuntu Router<br>шлюз 192.168.203.1]
        NATgateway[? 192.168.86.2]
    end

    KaliNet[Сеть Kali<br>192.168.203.0/24] --> Router
    Router --> NATgateway
    NATgateway --> NATnet
    KaliNet --> Kali[Kali Linux<br>192.168.203.128]
```
