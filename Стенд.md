...
| Имя VM          | ОС                | CPU (ядра) | RAM (ГБ) | Назначение                | Образ |
|----------------|-------------------|------------|----------|---------------------------|-------|
| Ubuntu Router | Ubuntu Server 20.04.6 | 2 | 4 | Маршрутизатор | ubuntu-20.04.6-live-server-amd64.iso |
| Windows Server | Windows Server 2022| 2 | 4  | AD, DNS, DHCP | ru-ru_windows_server_2022_updated_april_2026_x64_dvd_a4d60e24.iso |
| Windows Desktop | Windows 10 | 4 | 4 | Клиент LAN | ru-ru_windows_10_consumer_editions_version_22h2_updated_oct_2025_x64_dvd_38efd00d.iso |
| Kali Linux | Kali Linux | 4| 4 | Клиент KALI | kali-linux-2026.1-vmware-amd64 |


## Создание виртуальных сетей в VMware

В Virtual Network Editor созданы три:
- **VMnet1** (LAN) – подсеть `192.168.47.0/24`, DHCP отключён.
- **VMnet2** (DMZ) – подсеть `192.168.211.0/24`, DHCP отключён.
- **VMnet3** (KALI) – подсеть `192.168.203.0/24`, DHCP включён.

<img width="555" height="133" alt="{231AA096-E289-48DC-A5E8-D2345D0F6B69}" src="https://github.com/user-attachments/assets/b2157bfe-80bf-4598-8001-fa76669ffaa0" />
...
Панель управления (Просмотр: Категория) -> Сеть и интернет -> Центр управлениями сетями и общим доступ -> Изменение параметров адаптера -> заходим в свойства Ethernet -> IP версии 4 (TCP/IPv4)(Свойства)
...
**Создание областей для подсетей LAN (VMnet1), NET (VMnet2)**
```powershell
# LAN
...
# DMZ
Add-DhcpServerv4Scope -Name "DMZ" -StartRange 192.168.211.50 -EndRange 192.168.211.150 -SubnetMask 255.255.255.0 -LeaseDuration 8.00:00:00
...
<img width="1036" height="108" alt="{6D696F89-6CA4-4003-8E21-D0B75522385B}" src="https://github.com/user-attachments/assets/53c1affd-ef63-4b1c-a11e-f3e5257c6278" />
...
<img width="645" height="281" alt="{2E8A47CC-DF74-485A-B4F8-F63F06FD616F}" src="https://github.com/user-attachments/assets/3fce7395-ee7f-4700-9e40-83c1aa83f3db" />
...
<img width="811" height="100" alt="{941C020F-2D36-4FCC-9AC4-E8980AF04E83}" src="https://github.com/user-attachments/assets/7446a708-5330-4486-9c15-f2dadff89e94" />

## Ввод Windows Desktop в домен

Параметры -> Система -> О программе -> Дополнительные параметры системы -> Имя компьютера
...
- name: Настройка DHCP-областей
  ...
  tasks:
    - name: Создание DHCP-области LAN, DMZ
      win_shell: |
        # LAN
        ...
        # DMZ
        Add-DhcpServerv4Scope -Name "DMZ" -StartRange 192.168.211.50 -EndRange 192.168.211.150 -SubnetMask 255.255.255.0 -LeaseDuration 8.00:00:00 -ErrorAction SilentlyContinue
...
flowchart LR
    ...
    KALI1 <--> VMnet3[VMnet3: 192.168.203.0/24<br>DHCP встроенный VMware]
    VMnet3 <--> Kali[Kali Linux<br>192.168.203.128]

    Server -.-> |DHCP Relay| LAN1
    Server -.-> |DHCP Relay| DMZ1
```
...
| Ubuntu Router| DMZ (ens35 / VMnet2) | 192.168.211.1 | 255.255.255.0 | – | – |
| Ubuntu Router| Kali (ens36 / VMnet3) | 192.168.203.1 | 255.255.255.0 | – | – |
| Windows Server | DMZ (VMnet2) | 192.168.211.10 | 255.255.255.0 | 192.168.211.1 | 127.0.0.1 |
| Windows Desktop | LAN (VMnet1) | 192.168.47.50 | 255.255.255.0 | 192.168.47.1 | 192.168.211.10 |
| Kali Linux | KALI (VMnet3) | 192.168.203.128 | 255.255.255.0 | 192.168.203.1 | 192.168.203.1 |

Примечание: Kali получает IP от встроенного DHCP VMware (VMnet3), не зависит от Windows Server. Шлюзом является Ubuntu Router (192.168.203.1). Доступ в интернет обеспечивается через NAT на Ubuntu Router.

---

| Отправитель |Получатель | Протокол | Доступность |
|---------------|----------|----------------|-----------|
| Windows Desktop (LAN) | Windows Server (DMZ) | ICMP, SMB (445), RDP (3389), любые |  Да |
| Windows Desktop (LAN) | Kali (KALI) | ICMP, любые |  Да |
| Windows Desktop (LAN)| Интернет (8.8.8.8) | ICMP, HTTP/HTTPS (80,443) и др. |  Да | 
| Windows Server (DMZ) | Windows Desktop (LAN) | ICMP, SMB, RDP |  Да | 
| Windows Server (DMZ) | Kali (KALI) | ICMP, любые |  Да | 
| Windows Server (DMZ) | Интернет | ICMP, HTTP/HTTPS |  Да | 
| Kali (KALI) | Windows Server (DMZ) | ICMP, любые |  Да |
...
|--------|------|----------------|----------------------|------------|
| DNS | Windows Server (DMZ) | UDP/TCP 53 |  Только для LAN и DMZ | Преобразует доменные имена в IP‑адреса |
| DHCP | Windows Server (DMZ) | UDP 67,68 |  Только для LAN и DMZ | Через DHCP Relay для LAN; в DMZ – напрямую |
| Active Directory | Windows Server (DMZ) | LDAP (389), Kerberos (88), SMB (445) |  Для доменных клиентов | Только после ввода клиента в домен lab.local |
| WinRM (HTTP) | Windows Server (DMZ) | TCP 5985 |  С Ubuntu Router (Ansible) | - |
