...
| Windows Server | NET (VMnet2) | 192.168.211.10 | 255.255.255.0 | 192.168.211.1 | 127.0.0.1 |
...
|--------|------|----------------|----------------------|------------|
| DNS | Windows Server (NET) | UDP/TCP 53 |  Только для LAN и NET | Преобразует доменные имена в IP‑адреса |
| DHCP | Windows Server (NET) | UDP 67,68 |  Только для LAN и DMZ | Через DHCP Relay для LAN; в NET – напрямую |
...
| WinRM (HTTP) | Windows Server (NET) | TCP 5985 |  С Ubuntu Router (Ansible) | - |
